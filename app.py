...
@st.cache_data
def load_ipl_stats(selected_year):
    conn = sqlite3.connect('sports_analytics.db')
...
tab1, tab2 = st.tabs(["🔴 Live & Upcoming", "🏏 IPL"])
...
with tab2:
    st.subheader("🏆 IPL Database & Analytics")
    
    years = ["All-Time"] + [str(year) for year in range(2025, 2007, -1)]
    selected_year = st.selectbox("Select IPL Season:", years)
    
    df_history = load_ipl_stats(selected_year)
    
    st.markdown(f"**Top Run Scorers: {selected_year}**")
